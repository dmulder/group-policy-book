...
> samba-tool gpo admxload \
 --admx-dir=./samba-4.17.3/libgpo/admx -UAdministrator
```

::: {#warn style="color: red;"}
Warning: There is a bug in some versions of Samba that prevents the GNOME Settings ADMX templates from being displayed in the Group Policy Management Editor (GPME). Some versions of GPME are unable to read ADMX templates with a space in the file name. To correct the problem, rename the GNOME Settings files within your `libgpo/admx` and `libgpo/admx/en-US` (or `libgpo/admx/ru-RU` when installing Russian templates) to remove the space in the filename. For example, `libgpo/admx/GNOME Settings.admx` becomes `libgpo/admx/GNOME_Settings.admx`, and `libgpo/admx/en-US/GNOME Settings.adml` becomes `libgpo/admx/en-US/GNOME_Settings.adml`.
Ensure you then delete the erroneous filenames from your `SYSVOL`.
:::
