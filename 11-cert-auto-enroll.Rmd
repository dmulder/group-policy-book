...
[W05621]| The Network Device Enrollment Service is either not installed or not configured. | {}
...
Apply the changes and click OK to close the properties dialog. This has enabled simple Certificate Auto Enrollment. Next lets configure advanced Certificate Auto Enrollment.

Advanced Certificate Auto Enrollment allows you add additional certificate servers to your configuration. This is not mandatory in a simple environment with only one certificate server. The advanced configuration stores the policy directly on the SYSVOL in the Registry.pol file, whereas a simple configuration is stored in LDAP. Open the `Certificate Services Client - Certificate Enrollment Policy` properties dialog (also under `Computer Configuration > Policies > Windows Settings > Security Settings > Public Key Policies` in the GPME).
...
![Certificate Services Client - Certificate Enrollment Policy: Enabled](cert-auto-enroll-images/advanced-config-enabled.png)
...
Open the `Certification Authority` utility, and click on `Certificate Templates` in the tree.

![Certification Authority utility](cert-auto-enroll-images/certsrv.png)

Let's create a new template by right clicking on `Certificate Templates` in the tree, and selecting `Manage`. You'll notice that there are a number of existing templates to choose from. Right click on one of the templates in the list, and click `Duplicate Template`.

![Certificate Templates Console](cert-auto-enroll-images/cert-temp-console.png)

Switch to the `General` tab, and set a name for your new template. Set the number of years the certificate will be valid and the renewal period.

![Template Properties](cert-auto-enroll-images/template-properties.png)

It may be useful to also review the `Cryptography` tab and ensure the cryptographic requirements for the certificate will meet the needs of your organization.

Apply and close the properties by clicking OK. Close the Certificate Templates Console

Next, to enable the new template, right click on `Certificate Templates` in the tree within the Certification Authority utility, and click `New > Certificate Template to Issue`.

In the `Enable Certificate Templates` dialog, select the template we just created, and click OK.

![Enable Certificate Templates](cert-auto-enroll-images/enable-template.png)

Our new template is now enabled, and should be in the list of enabled templates in the Certification Authority utility.

![Template Enabled](cert-auto-enroll-images/template-enabled.png)

## Client Side Extension

The Certificate Auto Enrollment Client Side Extension (CSE) will add new Certificate Authorities to certmonger, and automatically request to track new certificates based on the assigned Certificate Templates.
