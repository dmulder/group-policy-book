...
![GNOME Settings Administrative Templates](gnome-images/gpme.png)

Let's set the allowed online accounts, which will effect the `Online Accounts` list in the GNOME Settings Dialog.

![Whitelisted Online Accounts](gnome-images/online-accounts.png)
...
> sudo tdbdump /var/lib/samba/gpo.tdb -k "GPTEST$" | sed -r "s/\\\22/\"/g" | xmllint --format -
<?xml version="1.0"?>
<gp>
  <user name="GPTEST$">
    <guid value="{31B2F340-016D-11D2-945F-00C04FB984F9}">
      <gp_ext name="GNOME Settings/Lock Down Settings">
        <attribute name="Whitelisted Online Accounts">/etc/dconf/db/local.d/0000000001-goa;/etc/dconf/db/local.d/locks/0000000001-goa</attribute>
      </gp_ext>
    </guid>
    <applylog>
...
```
> dconf dump / | grep -E "(whitelisted-providers|org/gnome/online-accounts)"
[org/gnome/online-accounts]
whitelisted-providers=['google']
```
...
![Online Accounts](gnome-images/online-accounts-gnome.png)
